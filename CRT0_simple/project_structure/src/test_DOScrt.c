...
char main(void) 
{
  char MSXsion;  //=PEEK(0x002D); //MSXVER · LINLEN
  char A=0;
//  uint i;
  
  PRINT("Test crt0 MSXDOS simple\n\r");
  
  MSXsion = ReadinBIOS(MSXVER);  
  PRINT("MSX ver:");
  switch(MSXsion) 
  {
	 case 1: PRINT("MSX2");break;
	 case 2: PRINT("MSX2+");break;
	 case 3: PRINT("turboR");break;
	 default: PRINT("MSX1");break;
  }
  
  ReadDOSversion();
  PRINT("\n\rDOS ver:");
  PrintNumber(vers_major);
  
  if(vers_major>1)
  {
    PRINT(".");
    PrintNumber(vers_minor);
  }
  
//  PRINT("\n\rEnd of MSXDOS TPA:");
//  PrintNumber(WPEEK(0x0006));
